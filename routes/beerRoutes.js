...
const { count } = require('../models/consumptionModel');
...
router.get('/:id', verify,async (req, res) =>{
    try {
        //const beer = await (await Beer.findById(req.params.id)).populate("consumptions");
        const beer = await Beer.findOne({_id: req.params.id});
        
        const user = await User.findById(req.user._id);

        const consumption = await Consumption.findOne({beer:beer._id,user:user._id});
        let consumptionText = "";
        ...
        if(!consumption){
            ...
            if(consumption.count===1){
                consumptionText="Usted ha consumido "+consumption.count+" cerveza "+beer.name;
            }else{
                consumptionText="Usted ha consumido "+consumption.count+" cervezas "+beer.name;
            }
        }
...
router.post('/consumption', verify,async (req, res) =>{
    ...
    if(consumption){
        ...
        const consumptionUpdated = await Consumption.findOne({beer:beer._id,user:user._id});
